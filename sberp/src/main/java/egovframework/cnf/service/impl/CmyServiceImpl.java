...
public class CmyServiceImpl implements CmyService {
  ...
  public List<CmdDTO.Sub> selectCmdList(ComParam param) {
    ...
    year.setMon("년 계");
    year.setInCnt(
        (int) result.stream().mapToInt(amt -> amt.getInCnt()).summaryStatistics().getSum());
    year.setInAmt(
        (long) result.stream().mapToLong(amt -> amt.getInAmt()).summaryStatistics().getSum());
  ...
  private void syncCmpyRelations(int childCmpySeq, List<Integer> prntCmpySeqList, String userId) {
    ...
    if (prntCmpySeqList == null)
      prntCmpySeqList = new ArrayList<>();

    // 3. INSERT 대상: 새로 들어온 값 중 DB에 없는 것
    List<Integer> insertList = prntCmpySeqList.stream().filter(p -> !existingList.contains(p))
        .collect(Collectors.toList());
