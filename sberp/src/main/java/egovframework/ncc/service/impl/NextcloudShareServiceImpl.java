...
public class NextcloudShareServiceImpl implements NextcloudShareService {
  ...
  private String findExistingShareToken(String path) throws Exception {
    String url = base + "/ocs/v2.php/apps/files_sharing/api/v1/shares" + "?path=" + encode(path)
  ...
  private void createShare(String path, String shareType, String shareWith, int permissions)
      throws Exception {
    String url = base + "/ocs/v2.php/apps/files_sharing/api/v1/shares";
  ...
  /**
   * 정책 B: - 사용자 공유(share_type=0)만 삭제 - 그룹공유(1), 공개링크(3), 이메일(4) 등은 유지
   */
  ...
  private List<Long> findShareIdsByPathUserOnly(String davPath) throws Exception {
    String url = base + "/ocs/v2.php/apps/files_sharing/api/v1/shares" + "?path="
        + URLEncoder.encode(davPath, "UTF-8") + "&format=json";
    ...
    JsonNode data = new ObjectMapper().readTree(body).path("ocs").path("data");
  ...
  private void deleteShareById(Long id) throws Exception {
    String url = base + "/ocs/v2.php/apps/files_sharing/api/v1/shares/" + id;
    ...
    int code = res.getStatusLine().getStatusCode();
    String body =
        res.getEntity() != null ? EntityUtils.toString(res.getEntity(), StandardCharsets.UTF_8)
            : "";
